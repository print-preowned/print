...
class BusinessRatingService:
    ...
    async def read(self, params: PaginationParams) -> PaginatedResponse[BusinessRatingRead]:
        total_results = await self._repo.count_business_ratings()
        rows = await self._repo.list_business_ratings(offset=params.offset, limit=params.size)
