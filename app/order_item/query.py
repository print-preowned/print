...
from .model import OrderItem, OrderItemCreateRequest, OrderItemUpdateRequest
import math
...
async def update_query(id: str, item: OrderItemUpdateRequest):
...
async def read_query(params: ParamRequest) -> PaginatedData[OrderItem]:
    ...
    return PaginatedData(
        data=[OrderItem.model_validate(record) for record in records],
...
async def read_by_id_query(id: str) -> OrderItem | None:
    record = await collection.find_one(
        {"_id": ObjectId(id), "status": {"$ne": "DELETED"}}
    )
    if not record:
        return None
    return OrderItem.model_validate(record)
