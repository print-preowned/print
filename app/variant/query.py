...
async def update_query(id: str, item: VariantUpdateRequest) -> UpdateResult:
    ...
    async with get_sessionmaker()() as session:
        updated = await VariantRepository(session).update_variant(
            parsed_id,
            VariantUpdate(**data),
        )
        if updated is None:
...
async def delete_query(id: str) -> UpdateResult:
    parsed_id = _parse_id(id)
    async with get_sessionmaker()() as session:
        deleted = await VariantRepository(session).soft_delete_variant(parsed_id)
        if not deleted:
            return UpdateResult(matched_count=0)
        await session.commit()
    return UpdateResult(matched_count=1)
...
async def read_query(params: ParamRequest) -> PaginatedData[VariantRead]:
    ...
    async with get_sessionmaker()() as session:
        total_results = await VariantRepository(session).count_variants()
        rows = await VariantRepository(session).list_variants(offset=offset, limit=size)
...
async def read_by_id_query(id: str) -> VariantRead | None:
    parsed_id = _parse_id(id)
    async with get_sessionmaker()() as session:
        row = await VariantRepository(session).read_variant_by_id(parsed_id)
...
async def read_by_business_book_id_query(
    ...
    async with get_sessionmaker()() as session:
        ...
        rows = await VariantRepository(session).list_variants(
            offset=offset,
            limit=size,
            business_book_id=parsed_bb_id,
        )
...
async def read_by_id_with_config_query(id: str) -> VariantWithConfigRead | None:
    ...
    async with get_sessionmaker()() as session:
        ...
        if row is None:
            return None
        config_map = await VariantRepository(session).resolve_configs_for_variants([row.id])
...
async def create_query(business_book_id: str, payload: VariantCreateRequest) -> str:
    ...
    async with get_sessionmaker()() as session:
        try:
            await VariantRepository(session).validate_product_option_values(value_ids)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc

        if await VariantRepository(session).duplicate_option_set_exists(parsed_bb_id, value_ids):
            raise HTTPException(
                status_code=409,
        ...
        created = await VariantRepository(session).create_variant(
            VariantCreate(
                business_book_id=parsed_bb_id,
...
async def read_public_catalog_query(
    ...
    async with get_sessionmaker()() as session:
        total_results = await VariantRepository(session).count_variants(active_catalog_only=True)
        rows = await VariantRepository(session).list_variants(
            offset=offset,
            limit=size,
        ...
        business_books = await BusinessBookRepository(session).read_business_books_by_ids(bb_ids)
        bb_by_id = {row.id: row for row in business_books}

        books = await BookRepository(session).read_books_by_ids(
            [bb.book_id for bb in business_books]
        )
        ...
        for row in rows:
            ...
            if book is None:
                continue
            business = await BusinessRepository(session).read_by_id(bb.business_id)
            if business is None:
                continue
            data.append(
                PublicCatalogVariantRead(
...
async def read_public_catalog_by_id_query(id: str) -> PublicCatalogVariantRead | None:
    ...
    async with get_sessionmaker()() as session:
        ...
        if row is None or row.status != "ACTIVE" or row.stock <= 0:
            return None
        bb_list = await BusinessBookRepository(session).read_business_books_by_ids(
            [row.business_book_id]
        )
        if not bb_list or bb_list[0].status != "ACTIVE":
            return None
        bb = bb_list[0]
        books = await BookRepository(session).read_books_by_ids([bb.book_id])
        if not books:
            return None
        book = books[0]
        business = await BusinessRepository(session).read_by_id(bb.business_id)
