...
db = get_database()
collection = db["inventory_item"]


async def create_query(item: InventoryItemCreateRequest):
...
async def update_query(id: str, item: InventoryItemUpdateRequest):
...
async def read_query(params: ParamRequest) -> PaginatedData[InventoryItem]:
    ...
    return PaginatedData(
        data=[InventoryItem.model_validate(record) for record in records],
...
async def read_by_id_query(id: str) -> InventoryItem | None:
    record = await collection.find_one(
        {"_id": ObjectId(id), "status": {"$ne": "DELETED"}}
    )
    if not record:
        return None
    return InventoryItem.model_validate(record)
