...
async def signup_query(user: SignupRequest) -> str:
    ...
    async with get_sessionmaker()() as session:
        created = await UserRepository(session).signup_user(payload)
...
async def create_query(user: UserCreateRequest) -> None:
    ...
    async with get_sessionmaker()() as session:
        await UserRepository(session).create_user(payload)
        await session.commit()
...
async def update_query(id: str, user: UserUpdateRequest) -> UpdateResult:
    ...
    async with get_sessionmaker()() as session:
        updated = await UserRepository(session).update_user(parsed_id, _to_user_update(user))
        if updated is None:
...
async def delete_query(id: str) -> UpdateResult:
    parsed_id = _parse_user_id(id)
    async with get_sessionmaker()() as session:
        deleted = await UserRepository(session).soft_delete_user(parsed_id)
        if not deleted:
            return UpdateResult(matched_count=0)
        await session.commit()
        return UpdateResult(matched_count=1)
...
async def read_query(params: ParamRequest) -> PaginatedData[UserRead]:
    ...
    async with get_sessionmaker()() as session:
        total_results = await UserRepository(session).count_users()
        rows = await UserRepository(session).list_users(offset=offset, limit=size)
...
async def read_by_id_query(id: str) -> UserRead | None:
    async with get_sessionmaker()() as session:
        row = await UserRepository(session).read_user_by_id(_parse_user_id(id))
...
async def read_by_ids_query(ids: list[str]) -> list[UserRead]:
    ...
    if not parsed_ids:
        return []
    async with get_sessionmaker()() as session:
        rows = await UserRepository(session).read_users_by_ids(parsed_ids)
    return [_to_read(row) for row in rows]


async def read_by_role_id_query(role_id: str) -> list[UserRead]:
    async with get_sessionmaker()() as session:
        rows = await UserRepository(session).read_users_by_role_id(uuid.UUID(role_id))
    return [_to_read(row) for row in rows]


async def read_by_email_query(email: str) -> UserRead | None:
    async with get_sessionmaker()() as session:
        row = await UserRepository(session).read_user_by_email(email)
    return _to_read(row) if row else None
