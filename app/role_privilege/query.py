...
async def create_query(mapping: RolePrivilegeCreateRequest) -> None:
    ...
    async with get_sessionmaker()() as session:
        await RolePrivilegeRepository(session).create_role_privilege(payload)
...
async def update_query(id: str, mapping: RolePrivilegeUpdateRequest) -> UpdateResult:
    ...
    async with get_sessionmaker()() as session:
        updated = await RolePrivilegeRepository(session).update_role_privilege(
            parsed_id,
...
async def delete_query(id: str) -> UpdateResult:
    parsed_id = _parse_id(id)
    async with get_sessionmaker()() as session:
        deleted = await RolePrivilegeRepository(session).soft_delete_role_privilege(parsed_id)
        if not deleted:
            return UpdateResult(matched_count=0)
        await session.commit()
    return UpdateResult(matched_count=1)
...
async def delete_by_role_and_privilege_query(role_id: str, privilege_code: str) -> UpdateResult:
    async with get_sessionmaker()() as session:
        deleted = await RolePrivilegeRepository(session).soft_delete_by_role_and_code(
            _parse_id(role_id),
            privilege_code,
        )
        if not deleted:
...
async def read_query(params: ParamRequest) -> PaginatedData[RolePrivilegeRead]:
    ...
    async with get_sessionmaker()() as session:
        total_results = await RolePrivilegeRepository(session).count_role_privileges()
        rows = await RolePrivilegeRepository(session).list_role_privileges(
...
async def read_by_id_query(id: str) -> RolePrivilegeRead | None:
    parsed_id = _parse_id(id)
    async with get_sessionmaker()() as session:
        row = await RolePrivilegeRepository(session).read_role_privilege_by_id(parsed_id)
...
async def read_by_role_and_privilege_query(
    ...
    async with get_sessionmaker()() as session:
        row = await RolePrivilegeRepository(session).read_role_privilege_by_role_and_code(
            _parse_id(role_id),
            privilege_code,
        )
    return _to_read(row) if row else None
...
async def read_by_privilege_code_query(privilege_code: str) -> list[RolePrivilegeRead]:
    async with get_sessionmaker()() as session:
        rows = await RolePrivilegeRepository(session).read_by_privilege_code(privilege_code)
    return [_to_read(row) for row in rows]


async def read_privilege_codes_by_role_id_query(role_id: str) -> list[str]:
    async with get_sessionmaker()() as session:
        return await RolePrivilegeRepository(session).read_privilege_codes_by_role_id(
            _parse_id(role_id)
        )
