...
db = get_database()
collection = db["platform_user"]


async def create_query(platform_user: PlatformUserCreateRequest):
    data = platform_user.model_dump()
...
async def update_query(id: str, platform_user: PlatformUserUpdateRequest):
    data = platform_user.model_dump(exclude_unset=True)
...
async def read_query(params: ParamRequest) -> PaginatedData[PlatformUser]:
    ...
    return PaginatedData(
        data=[PlatformUser.model_validate(record) for record in records],
...
async def read_by_id_query(id: str) -> PlatformUser | None:
